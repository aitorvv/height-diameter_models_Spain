...
#     df_adapted$boots_h <- predict(fit)
#     
#     boots_results <- bind_rows(boots_results, tibble(
#                                          n_boot = i,
#                                          aic = AIC(fit_boots), 
#                                          bic = BIC(fit_boots), 
#                                          logLik = fit_boots$logLik,
#                                          rmse = calculate_rmse(df_adapted$h, df_adapted$boots_h),
#                                          mae = calculate_mae(df_adapted$h, df_adapted$boots_h)))
#     pred_vs_obs <- bind_rows(pred_vs_obs, tibble(
#                                          n_boot = i,
#                                          n_tree = df_adapted$IFN_TREE_ID,
#                                          h = df_adapted$h,
#                                          boots_h = df_adapted$boots_h,
#                                          mb = df_adapted$h - df_adapted$boots_h))
#     
#   }} else {
#     print(paste('Model ', i, ' not fitted!'), sep = '')
#     boots_results <- bind_rows(boots_results, tibble(n_boot = i, 
#                                          aic = NA, bic = NA, logLik = NA, rmse = NA, mae = NA))
#     pred_vs_obs <- bind_rows(pred_vs_obs, tibble(n_boot = i, n_tree = df_adapted$IFN_TREE_ID, 
#                                                  h = df_adapted$h, boots_h = NA, mb = NA))
#   }
#   
#   # save results
#   save(boots_results, pred_vs_obs, file = paste('1_data/2_processed/2.6/2.6_cluster_bootstrap_results_', 
#                                                 i, '.rdata', sep = ''))
#   boots_results <- tibble(n_boot = numeric(), 
#                           aic = numeric(), bic = numeric(), logLik = numeric(), rmse = numeric(), mae = numeric())
#   pred_vs_obs <- tibble(n_boot = numeric(), n_tree = character(), h = numeric(), boots_h = numeric(), mb = numeric())
#   
# }
